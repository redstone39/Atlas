...
export function normalizeRoute(pathname: string): AppRoute | null {
  ...
  if (STATIC_ROUTES.has(pathname as StaticAppRoute)) {
    return pathname as StaticAppRoute;
  }
  ...
  try {
    if (
      ...
      const conversationId = decodeResourceId(segments[2]);
      return conversationId ? workspaceConversationRoute(conversationId) : null;
    }
    if (segments.length === 3 && segments[0] === "admin" && segments[1] === "users") {
      const actorId = decodeResourceId(segments[2]);
      return actorId ? adminUserDetailRoute(actorId) : null;
    }
    if (
      segments.length === 4 &&
      segments[0] === "admin" &&
      segments[1] === "teams" &&
      (segments[3] === "profile" || segments[3] === "members")
    ) {
      const teamId = decodeResourceId(segments[2]);
      return teamId ? adminTeamDetailRoute(teamId, segments[3]) : null;
    }
    if (
      segments.length === 4 &&
      segments[0] === "admin" &&
      segments[1] === "projects" &&
      (segments[3] === "profile" || segments[3] === "access")
    ) {
      const projectId = decodeResourceId(segments[2]);
      return projectId ? adminProjectDetailRoute(projectId, segments[3]) : null;
    }
    if (
      segments.length === 3 &&
      segments[0] === "admin" &&
      segments[1] === "audit" &&
      (segments[2] === "conversations" || segments[2] === "events")
    ) {
      return adminAuditSectionRoute(segments[2]);
    }
    if (
      segments.length === 5 &&
      segments[0] === "admin" &&
      ...
      const conversationId = decodeResourceId(segments[3]);
      return conversationId
        ? adminAuditConversationRoute(conversationId, "transcript")
        : null;
    }
    if (
      segments.length === 6 &&
      ...
      const conversationId = decodeResourceId(segments[3]);
      const turnId = decodeResourceId(segments[5]);
      return conversationId && turnId
        ? adminAuditConversationRoute(conversationId, "runtime", turnId)
        : null;
    }
  } catch {
    return null;
